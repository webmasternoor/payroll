...
<div class="container" xmlns:width="http://www.w3.org/1999/xhtml">
    ...
    <div class="panel-payscale panel-success" style="background:     ">
        ...
        <div class="panel-heading">Additional Allowance and Increment</div>
        <?php
        // echo form_open('account/update_status');
        //percentage initializaiton
        //        $medval = $this->db->where('item','%med%');
        $this->db->where('item','aya_allow');
        $conval = $this->db->get('percentage')->result_array();
        foreach($conval as $row12):
            $aya_per = $row12['percent'];
            $aya_item = $row12['item'];
        endforeach;
        $this->db->where('item','increment');
        $conval = $this->db->get('percentage')->result_array();
        foreach($conval as $row12):
            $increment_per = $row12['percent'];
            $increment_item = $row12['item'];
        endforeach;
        $this->db->where('item','city_allow');
        $conval = $this->db->get('percentage')->result_array();
        foreach($conval as $row12):
            $city_allow_per = $row12['percent'];
            $city_allow_item = $row12['item'];
        endforeach;
        ...
        <table class="table_payscale table table-hover text-center table-responsive" border="2" align="center">
            ...
            <tr style="border:1px solid white;">
                ...
                <th>Designation</th>
                <th>Basic</th>
                <th><a href="<?php echo base_url('payscale/edit_percentage/'.$city_allow_item);?>" class="btn btn-warning btn-sm">City/Hill/Island Allow (<?php echo $city_allow_per ?>%)</a></th>
                <th><a href="<?php echo base_url('payscale/edit_percentage/'.$med_item);?>" class="btn btn-warning btn-sm">Credit Allow (<?php //echo $med_per ?>??%)</a></th>
                <th><a href="<?php echo base_url('payscale/edit_percentage/'.$con_item);?>" class="btn btn-warning btn-sm">Increment(<?php echo $increment_per ?>%)</a></th>
                <th><a href="<?php echo base_url('payscale/edit_percentage/'.$cpf_item);?>" class="btn btn-warning btn-sm">Aya Allow(<?php echo $aya_per ?>%)</a></th>
                <th><a href="<?php echo base_url('payscale/edit_percentage/'.$cpf_item);?>" class="btn btn-warning btn-sm">Bima</a></th>
            </tr>
...

            {
                ...
                <tr>
                    ...
                    <td><?php echo $data->designation;?></td>
                    <td><?php echo $data->basic ;?></td>
                    <td><?php echo $data->city_allow; ?></td>
                    <td><?php echo "???";?></td>
                    <td><?php echo $data->increment;?></td>
                    <td><?php
                        if($data->grade>=1 && $data->grade<=18)
                            echo $data-> aya_allow;
                        else echo "-";
                        ?>
                    </td>
                    <td><?php echo $data->bima;?></td>
                    <td>
                        <a href="<?php echo base_url('payscale/edit_payscale/'.$data->grade);?>" class="btn btn-warning btn-sm"><i class="fa fa-edit fa-2x"></i></a>
