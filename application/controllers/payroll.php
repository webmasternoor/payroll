...
class Payroll extends MY_Controller {
	...
	public function test_emp()
    {
        $data["results"] = $this->payrolls->get_employee_hr();
    ...
    {
        $data["results"] = $this->payrolls->get_employee_hr();
        $this->load->view('setup/employee_search',$data);
    }
	public function leave_count($check){
		...
		if($this->session->userdata('logged_in')){
            ...
            if ($check==2){
                foreach ($data["results"] as $key => $x) {

                    $days =  $this->input->post($x->PBI_ID);
                    $month = $this->input->post('month');
                    $year = $this->input->post('year');
                    $oc = $this->input->post($key);
                    $submit = $this->input->post('submit');
                    $update = $this->input->post('update');

                    $this ->db->select('*');
                    $this ->db->from('ded_day');
                    $this->db->where('account_id',$x->PBI_ID);
                    ...
                    if ($query->num_rows() > 0 && $submit) {
                        ...
                        return ;
                            ...
                            $joining_date = DateTime::createFromFormat('Y-m-d', $x->PBI_DOJ);
                            ...
                            $insert = array('day' => $days, 'account_id' => $x->PBI_ID, 'oc'=>$oc,'month' => $month, 'year' => $year, 'ded_amount' => $ded_amount);
                            if ($submit)
	...
	public function setup_update($ID){
		...
		if($this->session->userdata('logged_in'))
		{
            $data["results"] = $this->payrolls->setup_updates($ID);

            redirect(base_url('setup_action',$ID));
        }
    ...
    public function payment(){
         ...
         if($this->session->userdata('logged_in')) {
            $data["results"] = $this->payrolls->get_employee_with_leave(null );
       	...
       	if($this->session->userdata('logged_in')){
            ...
            for($i = 0; $i < sizeof($check_id); $i++){
                    $data = array('account_id'=> $check_id[$i],
                            ...
                            'tax'=> $tax[$i],
                            'aya_allow'=> $aya[$i],
                            'credit_allow'=> $credit[$i],
                            'load_allow'=> $load[$i],
                            'city_allow'=> $city[$i],
                            'risk_allow'=> $risk[$i],
                            'distance_allow'=> $distance[$i],
                            'tech_allow'=> $tech[$i],
                            'savings'=> $savings[$i],
                            'dps'=> $dps[$i]
                    );
	...
	public function search(){
		...
		if($delimiters['keyword']!=null){
            ...
            $this->load->view('setup/search_result',$data);
	...
	public function setup_action($id)
	{
        if($this->session->userdata('logged_in'))
        {
            $data["records"] = $this->payrolls->profile($id);
    		$this->load->view('setup/setup_view',$data);
		}
		else{
			  redirect(base_url(''));
		}
